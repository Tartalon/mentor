...
cargoInputs.addEventListener("input", function (e) {
  ...
  totalPrice = getCostOfDelivery(inputWidth, inputHight, weight);
...
function getCostOfDelivery(width, height, weight) {
  const maxWeight = 500;
  let cost = 0;
  const costPerWidt = width > 1 ? (width - 1) * 1000 : 0;
  const costPerHeight = height > 1 ? (height - 1) * 1200 : 0;
  const costPerWeight = weight > maxWeight / 2 ? 3500 : 0;
  cost = costPerWidt + costPerHeight + costPerWeight;
  return cost;
}
